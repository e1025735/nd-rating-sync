...
| `cursor.go` | `syncCursor` (resumable position carried in the scheduler payload), `parseCursor` / `marshal`, `callBudget` (10 s — see *Sync execution* for why this is much lower than Navidrome's hardcoded 30 s limit), and `deadlineCheckEvery` (8 — songs between deadline checks, used to cap the WASI clock-import frequency in the hot loop) |
...
- Forward progress is guaranteed: `processPairChunk` checks the deadline every `deadlineCheckEvery` (8) songs, so every callback advances the cursor by ≥1 song; a continuation chain always terminates. `setRating` idempotency makes a re-processed song on resume harmless.
- **Why callBudget is 10 s, not 20 s.** Production saw a wazero host-side panic (nil pointer deref in `Context.Walltime`) ~16 s into a callback: Navidrome's runtime is configured with `WithCloseOnContextDone(true)`, so any sub-30 s cancellation tears down the wasi sys context and the next `clock_time_get` call panics — and a host panic is **not** recoverable from inside the guest. The 10 s budget keeps us comfortably under any plausible host timeout. Checking the deadline every 8 songs (not every song) also cuts WASI clock-import frequency by an order of magnitude in the hot loop, reducing the surface area for the same panic.
