...
func TestExtractStarsFromFile_UnsupportedFormat(t *testing.T) {
	...
	stars, ok := extractStarsFromFile(f.Name(), "wav", defaultTagOrder)
	assert.False(t, ok)
	assert.Equal(t, 0, stars)
}
...
func TestExtractStarsFromFile_FLACWithFMPS(t *testing.T) {
	...
	stars, ok := extractStarsFromFile(path, "flac", []string{"MediaMonkey"})
	assert.True(t, ok)
...
func TestExtractStarsFromFile_OggVorbisWithFMPS(t *testing.T) {
	...
	stars, ok := extractStarsFromFile(path, "ogg", []string{"MediaMonkey"})
	assert.True(t, ok)
...
func TestExtractStarsFromFile_OpusWithFoobar(t *testing.T) {
	...
	stars, ok := extractStarsFromFile(path, "opus", []string{"foobar2000"})
	assert.True(t, ok)
...
func TestExtractStarsFromFile_ExtensionFromPath(t *testing.T) {
	...
	assert.False(t, ok, "aiff should be skipped as unsupported")
}
