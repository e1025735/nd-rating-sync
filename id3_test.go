...
func TestParseID3v2Rating_foobar2000VsMediaMonkeyOrder(t *testing.T) {
	// Both TXXX frames present: FMPS_Rating=0.4 (MediaMonkey → 2 stars),
	// RATING=5 (foobar2000 → 5 stars). Priority order picks the winner.
	tag := id3v2.NewEmptyTag()
	tag.AddFrame("TXXX", id3v2.UserDefinedTextFrame{
		Encoding: id3v2.EncodingUTF8, Description: "FMPS_Rating", Value: "0.4",
	})
	tag.AddFrame("TXXX", id3v2.UserDefinedTextFrame{
		Encoding: id3v2.EncodingUTF8, Description: "RATING", Value: "5",
	})
	var buf bytes.Buffer
	_, err := tag.WriteTo(&buf)
	require.NoError(t, err)
